...
class OptimalController(LLMDesignedMethod):
    """
    Interleaved Branch-Complete (IBC) Controller.

    Core idea — depth-first with adaptive width:
      Sequential branch completion + confidence-gated early stop, but with
      genuine 2D adaptivity:

      * Width adaptivity: Opens an initial batch of `n_init` branches, all
        active simultaneously.  Probes them round-by-round.  As branches
        complete (probe_more returns is_finish=True), their final answers
        enter a confidence pool.  When the confidence pool reaches threshold,
        stops immediately — NOT waiting for the remaining active branches to
        finish, saving their tail-token budget.  Adds new branches only when
        confidence is insufficient and budget remains.

      * Depth adaptivity: After warm_up rounds, tracks how many consecutive
        rounds each unfinished branch's current intermediate answer has
        disagreed with a *completed-answers* plurality.  Branches that
        persistently disagree with the completed-pool winner are abandoned
        early (saving their remaining depth).  Unlike Parallel_Probe, the
        abandonment signal is anchored to the *reliable* completed pool, not
        to noisy intermediate answers.

      * Conservative anchor (beta=1): warm_up is long, min_complete is high,
        conf_thresh is high, max_branch approaches the full 64 ceiling.

      * Low budget (beta=0): small n_init, early confidence gate (low thresh),
        quick start — saves tail budget of branches once 2 agree.

    Core mechanism novel vs seeds:
      - ASC / ESC: full reads only; no mid-branch abandonment.
      - Parallel_Probe: abandons based on intermediate-answer disagreement
        from step 0; no separate completed-answer pool for the stop signal.
      - IBC: separates "completed-pool confidence" from "abandonment signal":
        * Confidence is computed over final completed answers only (robust).
        * Abandonment is guided by disagreement with the completed-pool winner
          (more reliable than raw intermediate-answer plurality).
        This two-signal design is the key novel mechanism.

    beta schedule (single knob in [0, 1]):
      All hyperparameters are smooth analytic functions of beta.
      beta = 0 -> conservative, small budget; beta = 1 -> near-full budget.
    """
    ...
    _MAX_BRANCH = 64
    _MAX_OUTER_STEPS = 500  # hard safety cap
    ...
    def _schedule(self, beta: float) -> dict:
        ...
        All schedules are non-decreasing in beta (more budget as beta grows):
          ...
          warm_up          = round(2 + 8 * beta)           [2, 10]
                             Rounds before confidence gate and abandonment fire;
                             high beta = longer warm-up = more evidence gathered

          min_complete     = max(2, round(2 + 4 * beta))   [2, 6]
                             Minimum completed branches before stop is allowed

          conf_thresh      = 0.85 + 0.12 * beta             [0.85, 0.97]
                             Beta-majority confidence needed to stop;
                             higher = harder to stop = more budget

          n_init           = max(2, round(2 + 6 * beta))   [2, 8]
                             Initial branches opened simultaneously

          max_branch_use   = round(4 + 60 * beta)           [4, 64]
                             Total branch budget ceiling

          abandon_patience = max(4, round(5 + 7 * beta))   [5, 12]
                             Consecutive rounds of disagreement with the
                             completed-pool winner before abandonment;
                             high beta = more patient = fewer premature exits
        """
        ...
        max_branch_use   = min(self._MAX_BRANCH, round(4 + 60 * b)) # [4, 64]
        abandon_patience = max(4, round(5 + 7 * b))                 # [5, 12]
    ...
    # ------------------------------------------------------------------
    # Trace surface (matching seed style)
    # ------------------------------------------------------------------
    ...
    def _pool_stats(self, completed: List[str]) -> tuple:
        """
        Return (winner, confidence) over the completed-answer pool.
        Returns (None, 0.0) if pool is empty.
        """
        if not completed:
            return None, 0.0
    ...
    def solve(self, question) -> Optional[str]:
        ...
        self._trace_step(
            ...
            decision="start interleaved branch-complete control",
        )

        # Branch state:
        #   index        : branch_index from probe_new
        #   latest_ans   : most recent probe answer
        #   finished     : bool — branch reached final answer via probe_more
        #   abandoned    : bool — abandoned before finishing
        #   pool_disagree: consecutive rounds disagreeing with completed-pool winner
        branches: List[Dict[str, Any]] = []
        completed_answers: List[str] = []  # only from naturally-finished branches
        total_spawned = 0

        # ---- Phase 0: open initial batch of branches ----
        ...
        self._trace_step(
            ...
            step_output={"n_spawned": total_spawned, "n_completed": len(completed_answers)},
        ...
        while outer_step < self._MAX_OUTER_STEPS:
            ...
            # ---- Pool winner from completed answers ----
            pool_winner, pool_conf = self._pool_stats(completed_answers)

            # ---- Probe each active (unfinished, unabandoned) branch once ----
            ...
            for br in branches:
                ...
                if out is None:
                    # Branch exhausted — treat last answer as final
                    br["finished"] = True
                    if br["latest_ans"] is not None:
                        completed_answers.append(br["latest_ans"])
                        newly_completed.append(br["latest_ans"])
                    continue
                ...
                if is_finish:
                    ...
                    newly_completed.append(new_ans)

            # Update pool stats after probing
            pool_winner, pool_conf = self._pool_stats(completed_answers)
            n_complete = len(completed_answers)

            # ---- Update pool_disagree counters (post-warm_up, pool_winner known) ----
            n_unfinished = sum(
                1 for br in branches
                if not br["abandoned"] and not br["finished"]
            )
            if outer_step >= self.warm_up and pool_winner is not None and n_unfinished > 0:
                for br in branches:
                    if br["abandoned"] or br["finished"]:
                        continue
            ...
            # ---- Abandon persistently-deviating branches ----
            # Only if 2+ unfinished branches remain *after* abandonment
            abandoned_this_round: List[int] = []
            if outer_step >= self.warm_up and pool_winner is not None:
                ...
                # Keep at least 2 unfinished branches after abandonment
                n_keep = max(0, n_unfinished - len(cands))
                if n_keep >= 2:
                    # All candidates can be abandoned safely
                    for br in cands:
                        br["abandoned"] = True
                        abandoned_this_round.append(br["index"])
                elif n_unfinished > 2:
                    # Abandon the most deviant ones, keep 2
                    max_abandon = n_unfinished - 2
                    cands_sorted = sorted(cands, key=lambda b: -b["pool_disagree"])
                    for br in cands_sorted[:max_abandon]:
                        br["abandoned"] = True
                        abandoned_this_round.append(br["index"])

            n_active_total = sum(1 for br in branches if not br["abandoned"])

            self._trace_step(
                event="forward",
                goal="probe all active branches once",
                step_input={
                ...
                step_output={
                    ...
                    "abandoned_this_round": abandoned_this_round,
                },
                state={
                    "n_active_total": n_active_total,
                    "total_spawned":  total_spawned,
                },
                decision="check confidence and widening",
            )
            ...
            self._trace_step(
                ...
                state={"total_spawned": total_spawned},
                decision="early stop if all conditions met",
            )
            ...
            if (
                ...
                self._trace_step(
                    ...
                    state={"total_spawned": total_spawned},
                    decision=f"early stop: conf={pool_conf:.3f} >= thresh",
                )
                return pool_winner
            ...
            # ---- Widen: add a new branch if budget available and conf low ----
            ...
            self._trace_step(
                event="update_states",
                goal="widen state snapshot",
                step_input={"outer_step": outer_step, "want_widen": want_widen},
                step_output={
                    "total_spawned":  total_spawned,
                    "max_branch_use": self.max_branch_use,
                    "pool_conf":      round(pool_conf, 4),
                ...
                state={"n_active_total": n_active_total},
        ...
        self._trace_step(
            ...
            state={"total_spawned": total_spawned},
            decision="return majority of completed answers",
        )
        return final_winner